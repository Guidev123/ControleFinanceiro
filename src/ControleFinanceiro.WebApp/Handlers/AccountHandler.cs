...
namespace ControleFinanceiro.WebApp.Handlers
{
    public class AccountHandler(IHttpClientFactory _httpClientFactory) : IAccountHandler
    {
        ...
        public async Task<Response<string>> LoginAsync(LoginCommand command)
        {
            ...
            return result.IsSuccessStatusCode ? new Response<string>("Login realizado!", 200, "") : new Response<string>(null, 400, "Usuario ou senha inválidos");
        }
        public async Task<Response<string>> RegisterAsync(RegisterCommand command)
        {
            ...
            return result.IsSuccessStatusCode ? new Response<string>("Cadastro realizado!", 201, "") : new Response<string>(null, 400, "Não foi possivel criar sua conta");
        }
        public async Task LogoutAsync()
        {
            var emptyContent = new StringContent("{}", Encoding.UTF8, "application/json");
            var result = await _client.PostAsJsonAsync("/api/identity/logout", emptyContent);
